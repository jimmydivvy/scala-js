...
import org.scalajs.core.ir.Printers.IndentationManager
...
class TSDPrinter(protected val out: Writer) extends IndentationManager {
  ...
  def print(tree: Tree): Unit = {
    tree match {
      ...
      case tree: ClassDef =>
        ...
        val (jsClassName) = Definitions.decodeClassName(name.name)

        val parts = jsClassName.split("\\.")
...
//        print(s"declare module ${packageName} {\n")
//        print
//        print("}")
...
//        printColumn(defs, "{", "", "}")
      ...
      case tree: MethodDef =>
        ...
        val (jsMethodName,_,_) = Definitions.decodeMethodName(name.name)

        System.out.println("JSMethod Name is: " + jsMethodName)
